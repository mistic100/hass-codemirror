 ...
 * - Window.blueprintStudio API for external access
 *
 * COMMON PATTERNS:
 ...
 *
 * GLOBAL API:
 * - window.blueprintStudio.performGlobalSearch()
 * - window.blueprintStudio.performGlobalReplace()
      ...
      html += `
          <div class="search-result-file">
              ...
              <div class="search-result-list" id="results-${safeId}" style="display: block;">
                  ${matches.map(m => `
                      <div class="search-result-match" onclick="window.blueprintStudio.openFileAndScroll('${m.path.replace(/'/g, "\\'")}', ${m.line})" style="padding: 6px 12px 6px 34px; cursor: pointer; font-family: monospace; font-size: 12px; border-bottom: 1px solid var(--border-color); display: flex;">
...
export function initGlobalSearchHandlers() {
  ...
  window.blueprintStudio.openFileAndScroll = async (path, line) => {
  ...
  window.blueprintStudio.copyEntityId = (entityId) => {
