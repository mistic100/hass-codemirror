 ...
 *
 * PURPOSE:
 * Handles all UI rendering and user interactions for the Git integration panel.
 * Displays git status, file changes, branches, and provides UI controls for
 * staging, committing, pushing, and pulling.
 *
 * EXPORTED FUNCTIONS:
 * - registerGitUICallbacks(cb) - Register dependencies from app.js
 * - updateGitPanel() - Refresh the entire git panel UI
 * - renderGitFiles(container) - Render git file list with groups
 * - toggleGitGroup(groupKey, panelType) - Expand/collapse file groups
 * - toggleFileSelection(file) - Select/deselect file for staging
 * - stageSelectedFiles() - Stage all selected files
 * - stageAllFiles() - Stage all modified files
 * - unstageAllFiles() - Unstage all staged files
 * - commitStagedFiles() - Show commit dialog and commit
 * - applyVersionControlVisibility() - Show/hide git panel based on settings
 *
 * REQUIRED CALLBACKS (from app.js):
 * - isGitEnabled: Check if git is enabled
 * - gitStatus: Refresh git status from server
 * - gitStage: Stage files
 * - gitUnstage: Unstage files
 * - gitCommit: Commit with message
 * - saveSettings: Save settings to server
 ...
 * - abortGitOperation: Abort current git operation
 ...
 *
 * 1. Adding a new git file group:
 *    - Add group key to the groups array in renderGitFiles()
 *    - Add condition to filter files for that group
 *    - Add icon and label for the group
 *    - Group will automatically collapse/expand with existing logic
 *
 * 2. Adding a new git action button:
 *    - Add button HTML in updateGitPanel()
 *    - Add click handler in updateGitPanel() event listeners
 *    - Call appropriate git operation callback
 *    - Use setButtonLoading() for async operations
 *
 * 3. Adding a new git status display:
 *    - Modify updateGitPanel() to add new status info
 *    - Access gitState properties (branch, ahead, behind, etc.)
 *    - Update HTML structure in the panel
 *
 * 4. Adding file action context menu:
 *    - Already supports stage/unstage/discard/view diff
 *    - Add new action in renderGitFiles() file item event handler
 *    - Call appropriate callback for the action
 *
 * INTEGRATION POINTS:
 * - state.js: gitState (branch, ahead, behind, status, files, etc.)
 * - git-operations.js: All git command operations (via callbacks)
 * - git-diff.js: Diff viewing (showDiffModal)
 * - app.js: Provides all git operation callbacks
 *
 * STATE MANAGEMENT:
 * - gitState.collapsedGroups: Set of collapsed group keys
 * - gitState.selectedFiles: Set of selected file paths
 * - gitState.branch: Current branch name
 * - gitState.ahead: Number of commits ahead of remote
 * - gitState.behind: Number of commits behind remote
 * - gitState.status: Object with file groups (modified, staged, etc.)
 *
 * ARCHITECTURE NOTES:
 * - This module only handles UI - no git operations
 * - Git operations are in git-operations.js
 * - File groups auto-collapse/expand with saved state
 * - Checkboxes for multi-select staging
 * - Color-coded file status (green=staged, yellow=modified, red=deleted)
 *
 * COMMON PATTERNS:
 * - Check if git is enabled: if (!callbacks.isGitEnabled()) return;
 * - Refresh after action: await callbacks.gitStatus();
 * - Loading states: callbacks.setButtonLoading(btn, true/false);
 * - Toast notifications: callbacks.showToast(message, type);
 *
 * ============================================================================
 */
...
let callbacks = {
  ...
  repairBranchMismatch: null,
  abortGitOperation: null,
...
export function registerGitUICallbacks(cb) {
...
export function updateGitPanel() {
  ...
  if (!callbacks.isGitEnabled || !callbacks.isGitEnabled()) {
  ...
  if (gitState.totalChanges > 0 || branchWarningHtml || stuckWarningHtml || remoteCleanupHtml || divergedWarningHtml) {
    ...
    if (btnRepair && callbacks.repairBranchMismatch) {
      btnRepair.addEventListener("click", callbacks.repairBranchMismatch);
    }

    const btnAbort = document.getElementById("btn-abort-git");
    if (btnAbort && callbacks.abortGitOperation) {
      btnAbort.addEventListener("click", callbacks.abortGitOperation);
    }

    const btnDeleteRemote = document.getElementById("btn-delete-remote-master");
    if (btnDeleteRemote && callbacks.deleteRemoteBranch) {
      btnDeleteRemote.addEventListener("click", () => callbacks.deleteRemoteBranch("master"));
    }

    const btnForcePush = document.getElementById("btn-force-push");
    if (btnForcePush && callbacks.forcePush) {
      btnForcePush.addEventListener("click", callbacks.forcePush);
    }

    const btnHardReset = document.getElementById("btn-hard-reset");
    if (btnHardReset && callbacks.hardReset) {
      btnHardReset.addEventListener("click", callbacks.hardReset);
    }
  } else {
    container.innerHTML = `
      <div class="git-empty-state">
...
export async function stageSelectedFiles() {
  ...
  if (selectedFiles.length === 0) {
    if (callbacks.showToast) {
      callbacks.showToast("No files selected. Check boxes next to files you want to stage.", "warning");
    }
    return;
  }

  if (callbacks.gitStage) {
    await callbacks.gitStage(selectedFiles);
  }
...
export async function stageAllFiles() {
  ...
  if (unstagedFiles.length > 0 && callbacks.gitStage) {
    await callbacks.gitStage(unstagedFiles);
  }
}
...
export async function unstageAllFiles() {
  if (gitState.files.staged.length > 0 && callbacks.gitUnstage) {
    await callbacks.gitUnstage(gitState.files.staged);
  }
}
...
export async function commitStagedFiles() {
  ...
  if (!callbacks.showModal) return;

  const commitMessage = await callbacks.showModal({
  ...
  if (callbacks.gitCommit) {
    await callbacks.gitCommit(commitMessage);
  }
}
