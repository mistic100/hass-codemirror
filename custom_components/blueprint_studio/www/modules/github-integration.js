 ...
 *
 * PURPOSE:
 * Handles GitHub-specific operations: OAuth authentication, repository creation,
 * remote setup, and GitHub-specific git operations.
 *
 * EXPORTED FUNCTIONS:
 * - registerGithubIntegrationCallbacks(cb) - Register dependencies
 * - githubCreateRepo(name, isPrivate) - Create GitHub repository
 * - gitAddRemote(url, name) - Add remote repository
 * - showGithubSettings() - Show GitHub settings dialog
 *
 * HOW TO ADD FEATURES:
 * 1. Add GitHub API calls: Use fetchWithAuth with GitHub API endpoints
 * 2. Add OAuth scopes: Update GitHub App configuration
 * 3. Add repo operations: Create, delete, update settings
 * 4. Add collaboration features: Add collaborators, manage access
 *
 * INTEGRATION: git-operations.js, api.js, ui.js
 * COMMON PATTERNS: await githubCreateRepo(name), gitAddRemote(url)
 *
 * ============================================================================
 */
...
export function registerGithubIntegrationCallbacks(callbackMap) {
...
export async function repairBranchMismatch() {
  ...
  try {
    ...
    if (callbacks.gitStatus) await callbacks.gitStatus(true);
  } catch (e) {
    hideGlobalLoading();
...
export async function showGitExclusions() {
  return new Promise(async (resolve) => {
    ...
    try {
      ...
      try {
        const data = await callbacks.loadFile(".gitignore");
      ...
      const saveHandler = async () => {
        ...
        const success = await callbacks.saveFile(".gitignore", newContent);
...
export async function showGitSettings() {
  // Get current remotes
  const remotes = await callbacks.gitGetRemotes();
  ...
  if (btnGitInitModal) {
    btnGitInitModal.addEventListener("click", callbacks.gitInit, { once: true });
  }
  if (btnSaveGitRemote) {
  ...
  if (btnCleanGitLocks) {
    btnCleanGitLocks.addEventListener("click", async () => {
      ...
      const success = await callbacks.gitCleanLocks();
      if (success) {
  ...
  if (callbacks.isMobile && callbacks.isMobile()) {
...
export async function saveGitCredentials() {
  ...
  if (callbacks.gitSetCredentials) {
    await callbacks.gitSetCredentials(username, token, rememberMe);
  }
}
